...
--     and RLS policy are ensured. (This is the expected case.)
...
--   • The RLS policy is recreated in uuid-safe form (cast both sides),
--     which is REQUIRED after a text→uuid conversion or the old
--     `auth.uid()::text = user_id` policy would error on every query.
-- The entire file runs as one transaction: all-or-nothing.
-- ============================================================

-- ── 1. Convert user_id text → uuid (only if needed, validated first) ──
DO $$
DECLARE
  col_type  text;
  bad_count bigint;
...
-- ── 2. Add the FK to auth.users ON DELETE CASCADE (if not already there) ──
DO $$
BEGIN
  IF NOT EXISTS (
...
-- ── 3. Recreate the RLS policy in uuid-safe form ──
...
-- ── 4. Record in the migration ledger (no-op if 037 not yet applied) ──
DO $$
BEGIN
  IF EXISTS (
