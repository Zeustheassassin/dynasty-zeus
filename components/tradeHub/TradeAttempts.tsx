...
function TradeAttempts({
  ...
  if (!selectedLeague) return (
    <p className="text-slate-400 text-sm">Select a league from the dropdown above to view attempted trades.</p>
  );
  ...
  return (
    <div className="space-y-4">
      <Card>
        <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">Attempted Trades</div>
        <div className="mt-1 text-sm text-slate-200">
          Track trades you&apos;ve offered in <strong className="text-slate-100">{selectedLeague.name}</strong>. Mark outcomes to suppress duplicate suggestions and build owner profiles.
        </div>
        <div className="mt-3">
          <input
            type="text"
            ...
            className="w-full rounded-lg bg-slate-800 border border-slate-700 px-3 py-2 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500"
          />
        </div>
      </Card>
        ...
        return (
          <Card key={attempt.id}>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              <div className="ml-auto flex flex-col items-end text-[10px] text-slate-500">
            ...
            {isShowingCounter && (() => {
              ...
              const renderColumn = (
                ...
                const accentBg = side === "give" ? "bg-emerald-900/20 border-emerald-800/50" : "bg-red-900/20 border-red-800/50";
                const accentText = side === "give" ? "text-emerald-400" : "text-red-400";
                const hoverBg = side === "give" ? "hover:bg-emerald-900/30" : "hover:bg-red-900/30";
                return (
                  <div className="flex flex-col gap-2">
                        ...
                        {chosenPlayers.map((p) => (
                          <div key={p.player_id} className={`flex items-center justify-between rounded-lg border px-2 py-1.5 ${accentBg}`}>
                            ...
                            <div className="flex items-center gap-2 shrink-0 ml-1">
                              {p.value > 0 && <span className="text-[10px] font-mono text-slate-400">{p.value.toLocaleString()}</span>}
                              <button onClick={() => removePlayer(p.player_id, side)} className="text-slate-600 hover:text-red-400 text-xs">✕</button>
                            </div>
                          </div>
                        ))}
                        {chosenPicks.map((pk) => (
                          <div key={pk.key} className={`flex items-center justify-between rounded-lg border px-2 py-1.5 ${accentBg}`}>
                            ...
                            <div className="flex items-center gap-2 shrink-0 ml-1">
                              {pk.value > 0 && <span className="text-[10px] font-mono text-slate-400">{pk.value.toLocaleString()}</span>}
                              <button onClick={() => removePick(pk.key, side)} className="text-slate-600 hover:text-red-400 text-xs">✕</button>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="rounded-lg border border-slate-700 bg-slate-800/40 overflow-hidden max-h-52 overflow-y-auto">
                      {availP.length === 0 && availK.length === 0 && (
                        <p className="text-xs text-slate-600 px-2 py-2">Nothing available</p>
                      )}
                      {availP.map((p) => (
                        <button
                          key={p.player_id}
                          onClick={() => addPlayer(p, side)}
                          className={`w-full flex items-center justify-between px-2.5 py-1.5 text-left transition border-b border-slate-700/50 last:border-0 ${hoverBg}`}
                        >
                          <div className="flex items-center gap-1.5 min-w-0">
                            ...
                            {p.team && <span className="text-[10px] text-slate-600">{p.team}</span>}
                          </div>
                          <span className="text-[10px] font-mono text-slate-400 shrink-0 ml-1">{calcValC(p.player_id).toLocaleString()}</span>
                        </button>
                      ))}
                      {availK.length > 0 && (
                        <div className="border-t border-slate-700 mt-0.5">
                          {availK.map((pk) => (
                            <button
                              key={pk.key}
                            ...
                            >
                              ...
                              <span className="text-[10px] font-mono text-slate-400 shrink-0 ml-1">{pk.value.toLocaleString()}</span>
                            </button>
                          ))}
                        </div>
                      )}
              ...
              return (
                <div className="mb-3 rounded-xl border border-orange-800 bg-orange-950/10 p-3 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {renderColumn("give",    "They Give",    "text-emerald-400", oppPlayers, oppPicks)}
