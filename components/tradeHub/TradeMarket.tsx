...
function TradeMarket({ fcTrendData, loadingFcTrends, onRefreshFcTrends }: TradeMarketProps) {
  ...
  const RefreshButton = ({ section }: { section: string }) => (
    <button
      ...
      className="ml-auto flex items-center gap-1.5 text-[11px] text-slate-500 hover:text-slate-300 disabled:opacity-40 transition"
    >
  ...
  const PlayerRow = ({ entry, metric }: { entry: FcTrendEntry; metric: React.ReactNode }) => (
    <div className="flex items-center justify-between py-2 border-b border-slate-800 last:border-0">
      ...
      <div className="flex items-center gap-4 shrink-0 ml-3">
        <span className="text-[11px] text-slate-400">Val <span className="text-slate-200">{entry.value.toLocaleString()}</span></span>
        {metric}
      </div>
    </div>
  );
  ...
  if (fcTrendData.length === 0 && !loadingFcTrends) return (
    <div className="text-center py-12 text-slate-500 text-sm">
      ...
      <p className="mt-1 text-xs text-slate-600">It will appear automatically once FantasyCalc values finish loading.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      {/* Sub-view nav */}
      <div className="flex justify-center gap-4 border-b border-slate-800 pb-0 overflow-x-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              <div className="flex items-center mb-1">
                <div>
                  <div className="text-sm font-bold text-slate-100">Top Rebuilding Targets</div>
                  <div className="text-[11px] text-slate-500">Higher dynasty value than redraft</div>
                </div>
                <RefreshButton section="Rebuilding Targets" />
              </div>
              <div className="mt-3 grid grid-cols-[1fr_auto_auto] gap-x-3 text-[10px] font-semibold uppercase tracking-wide text-slate-600 mb-1 px-1">
                <span>Player</span><span>Diff</span><span>Dyn Val</span>
              </div>
              {rebuilding.length === 0 && <p className="text-xs text-slate-500 mt-2">No data for this filter.</p>}
              {rebuilding.map((e, i) => (
            ...
            <Card>
              <div className="flex items-center mb-1">
                <div>
                  <div className="text-sm font-bold text-slate-100">Top Contending Targets</div>
                  <div className="text-[11px] text-slate-500">Higher redraft value than dynasty</div>
                </div>
              ...
              {contending.length === 0 && <p className="text-xs text-slate-500 mt-2">No data for this filter.</p>}
              {contending.map((e, i) => (
                <div key={e.sleeperId} className="flex items-center gap-2 py-2 border-b border-slate-800 last:border-0">
                  <span className="text-[11px] text-slate-600 w-5 shrink-0">{i + 1}.</span>
                  <span className={`text-[10px] font-bold w-7 shrink-0 ${POS_COLOR[e.position] ?? "text-slate-400"}`}>{e.position}</span>
        ...
        return (
          <Card>
            <div className="flex items-center mb-3">
              <div>
                <div className="text-sm font-bold text-slate-100">Trending Up — 30 Day</div>
                <div className="text-[11px] text-slate-500">Biggest dynasty value risers this month</div>
              </div>
              <RefreshButton section="Trending Up" />
            </div>
            {rows.length === 0 && <p className="text-xs text-slate-500">No data for this filter.</p>}
            {rows.map((e) => (
              <PlayerRow
                key={e.sleeperId}
                entry={e}
      ...
      {marketView === "TRENDING_DOWN" && (() => {
        ...
        return (
          <Card>
            <div className="flex items-center mb-3">
              <div>
                <div className="text-sm font-bold text-slate-100">Trending Down — 30 Day</div>
                <div className="text-[11px] text-slate-500">Biggest dynasty value fallers this month</div>
              </div>
      ...
      {marketView === "MOST_TRADED" && (() => {
        ...
        return (
          <Card>
            <div className="flex items-center mb-3">
              <div>
                <div className="text-sm font-bold text-slate-100">Most Traded</div>
                <div className="text-[11px] text-slate-500">Players appearing most frequently in recent trades</div>
              </div>
            ...
            {rows.map((e) => (
              <PlayerRow
                ...
                entry={e}
                metric={
                  <span className="text-[11px] font-semibold text-amber-400 w-20 text-right">
