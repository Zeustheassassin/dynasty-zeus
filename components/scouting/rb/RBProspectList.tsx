    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <button
          ...
          className={`px-3 py-1 rounded text-xs font-medium transition ${!draftYearFilter ? "bg-green-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}
        >All</button>
        {years.map((y) => (
          <button key={y} onClick={() => setDraftYearFilter(y)}
            className={`px-3 py-1 rounded text-xs font-medium transition ${draftYearFilter === y ? "bg-green-600 text-white" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}
          >{y}</button>
        ))}
        <button
      ...
      {showAdd && (
        <div className="mb-4 p-4 bg-gray-900 border border-gray-700 rounded-lg">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
            <input className="col-span-2 md:col-span-1 px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white text-sm placeholder-gray-500 focus:outline-none focus:border-green-500"
              placeholder="Player Name *" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
            <input className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white text-sm placeholder-gray-500 focus:outline-none focus:border-green-500"
              placeholder="School" value={form.school} onChange={(e) => setForm((f) => ({ ...f, school: e.target.value }))} />
            <input className="px-3 py-2 bg-gray-800 border border-gray-700 rounded text-white text-sm placeholder-gray-500 focus:outline-none focus:border-green-500"
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowAdd(false)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white text-sm rounded transition">Cancel</button>
          </div>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="space-y-1">
          {filtered.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => onSelectProspect(p)}
