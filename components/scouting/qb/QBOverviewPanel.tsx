    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
        {[
          ...
          { label: "Games",        value: gamesCount,        color: "text-gray-300" },
        ].map((s) => (
      ...
      {stats.handlingCharted > 0 && (
        <div className="p-4 bg-gray-900 rounded-lg border border-gray-800">
          <div className="text-xs text-gray-500 mb-3 flex flex-wrap items-baseline gap-3">
            <span>
              Pressure Handling <span className="text-gray-700">({stats.handlingCharted} of {stats.pressuredPlaysCount} pressured plays charted)</span>
            </span>
            {stats.handlingMissing > 0 && (
              <span className="text-yellow-500/70 text-[10px]">{stats.handlingMissing} missing handling data</span>
            )}
          </div>
            ...
            {PRESSURE_HANDLINGS.map(({ key, label }) => {
              ...
              return (
                <div key={key} className="p-3 bg-gray-800/50 rounded-lg text-center">
                  ...
                  <div className="text-xs text-gray-600 mt-0.5">{fmtPct(sharePct)} of handled</div>
