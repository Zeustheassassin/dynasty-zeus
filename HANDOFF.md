...
20. [Test suite scope (550 tests, 25 files)](#20-test-suite-scope-550-tests-25-files)
...
Neither chart has been visually verified in a browser yet (no browser tool available this or the prior session) — only tsc/lint/550 tests checked each time.
...
## 20. Test suite scope (550 tests, 25 files)
...
7. **In PowerShell** (not Git Bash): `npm run test` → confirm **550 tests pass** across 25 files. If you see "0 tests," you're in the wrong shell.
...
11. Skim [supabase/migrations/](supabase/migrations/) (currently through `044`) to understand the data model's evolution.
