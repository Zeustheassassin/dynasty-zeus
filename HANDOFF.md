...
20. [Test suite scope (589 tests, 29 files)](#20-test-suite-scope-589-tests-29-files)
...
> **Status as of 2026-07-16 (verified live against Supabase during the Phase J session):** `league_simulation_history` has **zero rows**. This is expected, not broken — the table/cron were deployed the same calendar day as this check, and the schedule is Tuesday-only (`0 9 * * 2`), so it hasn't reached its first scheduled run yet. The older `league-transactions` cron (same auth/Vercel-cron mechanism) has 7,600+ fresh rows, so there's no reason to suspect the infra itself is broken. Confirm real data exists after 2026-07-21 (the first Tuesday following deployment) before trusting the Simulator tab's Odds Trend chart or the Dashboard's Team Summary playoff-odds sparklines (Phase J) — both read this table and will correctly show an empty/"no history yet" state until then.
...
## 20. Test suite scope (589 tests, 29 files)
...
7. **In PowerShell** (not Git Bash): `npm run test` → confirm **589 tests pass** across 29 files. If you see "0 tests," you're in the wrong shell.
