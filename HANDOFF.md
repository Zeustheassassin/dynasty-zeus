...
20. [Test suite scope (557 tests, 26 files)](#20-test-suite-scope-557-tests-26-files)
...
Neither chart has been visually verified in a browser yet (no browser tool available this or the prior session) — only tsc/lint/557 tests checked each time.
...
Both charts will show only 1 point (or the old fallback) until the cron has run at least twice — there is no backfill, history starts accumulating from first deploy. `npm run test` → **557 tests, 26 files** (new [player-value-history.test.ts](__tests__/app/api/cron/player-value-history.test.ts) covers the auth/env guards and the value-parsing + upsert happy path, same shape as `simulation-history.test.ts`). Not yet committed; migration 047 needs to be run in the Supabase SQL editor before the cron/charts will show real data.
...
## 20. Test suite scope (557 tests, 26 files)
...
7. **In PowerShell** (not Git Bash): `npm run test` → confirm **557 tests pass** across 26 files. If you see "0 tests," you're in the wrong shell.
