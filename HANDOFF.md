...
|---|---|:---:|---|
| `DASHBOARD` | Dashboard | yes | Landing screen + Sleeper username connect. Once connected: a stat strip, a cross-league "Your Teams" summary (record + playoff-odds sparkline per league, Phase J), Value Movers + Recent Alerts panels, then the hub nav tiles ([components/Dashboard.tsx](components/Dashboard.tsx), [components/Dashboard/](components/Dashboard/)). The only hub usable before connecting Sleeper. |
