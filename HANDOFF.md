...
>
> *Last reviewed: 2026-08-21 (working tree on top of HEAD `d62a5d5`, uncommitted — fixed a real Season Simulator correctness bug reported via two screenshots comparing the Trade Calculator's "Preview in Simulator" panel against the live Season Simulator standings table for the same untraded roster in the same league: Playoff/Title odds and Expected Wins disagreed by double digits with no trade or roster change involved, and the user correctly argued the direction didn't even make sense (a just-completed real trade that should only have helped their odds). Ruled out a bug in the preview feature itself first: [useSimulatorState.ts](app/hooks/useSimulatorState.ts)'s `previewTradeSimulation("before")` call and the live `selectedLeagueSimulation` memo both read the same `rosters`/`simSalt`/etc. state, so within one continuous session they're provably identical — confirmed live via a throwaway Playwright script (not committed) logged in as [[reference_visual_qa_test_account]]: added a real trade, and the "before" baseline matched the live Simulator tab almost exactly (99.7%≈100% playoff, 23.1%≈23% title). The actual bug: `simSalt` ([useSimulatorState.ts#L77](app/hooks/useSimulatorState.ts#L77), the Monte Carlo seed every `simulateLeague()` caller shares) was `Math.random()`-seeded fresh on every page mount and never persisted, so a plain reload — no trade, no roster change — silently re-rolled every playoff/title/wins number app-wide; for a team not already pinned at 0%/100%, that reseed could swing double digits (confirmed via the same script: a mid-pack team's numbers differed noticeably across two fresh loads before the fix, and were byte-identical after). Fixed by persisting `simSalt` to `localStorage` (`simSalt_v1`) — the lazy `useState` initializer now reads-or-generates-and-writes it, and only `handleRunAllSims`'s explicit re-roll writes a new value, so odds now stay stable across reloads/navigation and only change when the user asks for a fresh sim or the roster/schedule actually changes. tsc clean, 635/31 tests green. Not committed.)*
>
...
20. [Test suite scope (614 tests, 30 files)](#20-test-suite-scope-614-tests-30-files)
...
## 20. Test suite scope (614 tests, 30 files)
...
7. **In PowerShell** (not Git Bash): `npm run test` → confirm **614 tests pass** across 30 files. If you see "0 tests," you're in the wrong shell.
